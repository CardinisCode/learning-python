...
def stars(movies, tvshows):
    performers_dict = {}
    sorted_performers_dict = {}
    
    #Creating a key for every actor - in both dictionaries:
    for list_of_actors in movies.values():
        for actor in list_of_actors:
            if not actor in performers_dict: 
                performers_dict[actor] = []   

    for list_of_actors in tvshows.values():
        for actor in list_of_actors:
            if not actor in performers_dict: 
                performers_dict[actor] = []         

    #Adding these keys to our dictionary performers_dict:
    for (movie, list_of_actors) in movies.items():
        for actor in list_of_actors: 
            performers_dict[actor].append(movie)     

    for (tvshow, list_of_actors) in tvshows.items(): 
        for actor in list_of_actors: 
            performers_dict[actor].append(tvshow)
...
# 'Elizabeth Banks': ['30 Rock', 'The Lego Movie'], 
# 'Alec Baldwin': ['30 Rock'], 
# 'Alison Brie': ['Community', 'How to Be Single', 'The Lego Movie'], 
# 'Tina Fey': ['30 Rock'], 
...
# 'Will Arnett': ['Arrested Development', 'The Lego Movie'], 
