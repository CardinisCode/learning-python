...
def calculate_score(student_answers_dict, correct_answers_dict):
    correct_answers = 0
    if not len(student_answers_dict) == len(correct_answers_dict):
        return -1
    
    for question_number in student_answers_dict: 
        if student_answers_dict[question_number] == correct_answers_dict[question_number]:
            correct_answers += 1
