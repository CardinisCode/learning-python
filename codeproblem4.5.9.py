...
def length_words(aString):
    ...
    list_of_punctuation = [".", ",", "?", "!", "'"]
    for punctuation in list_of_punctuation:
        if punctuation in aString:
            aString = aString.replace(punctuation, "")
    aString = aString.split()

    aStringDictionary = {}
    for word in aString:
        word_length = len(word)
        if not word_length in aStringDictionary:
            aStringDictionary[word_length] = []
        aStringDictionary[word_length].append(word)

    return aStringDictionary
