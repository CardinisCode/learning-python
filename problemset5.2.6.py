...
def binary_search_year(date_instances, year):
    date_instances.sort()
    if len(date_instances) == 0:
        return False

    else:
        middle = len(date_instances) // 2
        if date_instances[middle].year == year:
            return True
        elif year < date_instances[middle].year:
            return binary_search_year(date_instances[:middle], year)

        else:
            return binary_search_year(date_instances[middle + 1:], year)
