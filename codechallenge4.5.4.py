...
def modify_dict(aDictionary):
    list_of_items_to_be_deleted = []
    for (surname, first_name) in aDictionary.items():
        first_letter = surname[0]
        if first_letter.islower():
            list_of_items_to_be_deleted.append(surname)
    
    print("Dictionary before deleting")
    print(aDictionary)
    for item in list_of_items_to_be_deleted: 
        del aDictionary[item]
